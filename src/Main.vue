  ...
  <el-container class="window">
    <el-header class="header">
      <HeaderToolbar
        ...
        @show-anime-tool="openAnimeTool"
        @show-ico-tool="openIcoTool"
...
<script>
import os from "os";
import HeaderToolbar from "./main/HeaderToolbar.vue";
import AsideFormats from "./main/AsideFormats.vue";
import MainTasks from "./main/MainTasks.vue";
import FooterInfobar from "./main/FooterInfobar.vue";
import SettingDialog from "./main/SettingDialog.vue";
...
export default {
  ...
  mounted() {
    ...
    process.on("getTask", (taskKey, callback) => {
      const task = tasksMap[taskKey];
      if (task) {
        const { key, input, edit } = task;
        callback({ key, input, edit });
      }
    });
    process.on("updateTask", (newTask) => {
      const task = tasksMap[newTask.key];
      if (task) {
