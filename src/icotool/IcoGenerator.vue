...
import { openAcceptRule } from "@/util/imageFiles";
...
const allExtractSizes = iconSizes.map((size) => size.value);
const defExtractSizes = [16, 24, 32, 48, 64, 72, 96, 128, 256];

const iconMenu = new Contextmenu(generatorIconMenu);
const listMenu = new Contextmenu(generatorListMenu);
const iconSizesMenu = new Contextmenu(iconSizes);
...
export default {
  ...
  methods: {
    ...
    showIconSizesMenu(e) {
      iconSizesMenu.popup(e, this.iconSizesMenuCallback);
    },
    ...
    extract() {
      ...
      showSaveDialog({
        ...
        if (!filePath.endsWith(".ico")) `${filePath}.ico`;
