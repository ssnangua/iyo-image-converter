  ...
  <div class="window" :class="{ dragging: frameDragging, playing }">
    ...
    <div class="zoom-tool">
      <span
        ...
        :title="$t('animeTool.zoomIn')"
        ><i class="iconfont icon-zoom-in"></i
      ></span>
      <span
        class="icon-btn"
        @click="setZoom(zoom - 0.1)"
        :title="$t('animeTool.zoomOut')"
        ><i class="iconfont icon-zoom-out"></i
      ></span>
      <span class="icon-btn" @click="setZoom(1)" :title="$t('animeTool.zoom1')"
        ><i class="iconfont icon-zoom-1"></i
      ></span>
      <span
        class="icon-btn"
        @click="setZoom(fitZoom)"
        :title="$t('animeTool.zoomFit')"
...
import animeToolSetting from "@/preset/animeTool";
...
import { saveFramesAs, saveImage } from "@/anime/util";
import contextmenu from "@/anime/contextmenu";
import OptionsEditor from "@/component/OptionsEditor.vue";
import SubtitleDialog from "@/anime/SubtitleDialog.vue";
import InputDialog from "@/anime/InputDialog.vue";
import AnimeTips from "@/anime/AnimeTips.vue";
  ...
  methods: {
    setZoom(zoom) {
      ...
      const { offsetWidth, offsetHeight, scrollWidth, scrollHeight } =
        canvasContainer;
      const { width, height, scaleFactor } = this;
      this.centerH = (width * zoom) / scaleFactor < offsetWidth;
      this.centerV = (height * zoom) / scaleFactor < offsetHeight;
      this.$nextTick(() => {
        if (!this.centerH)
          canvasContainer.scrollLeft = (scrollWidth - offsetWidth) / 2;
        if (!this.centerV)
          canvasContainer.scrollTop = (scrollHeight - offsetHeight) / 2;
      });
    },
    ...
    updateFitZoom() {
      ...
      const { width, height, zoom, scaleFactor } = this;
      this.fitZoom = Math.min(
        ((offsetWidth - 2) / width) * scaleFactor,
        ((offsetHeight - 2) / height) * scaleFactor
      );
      this.centerH = (width * zoom) / scaleFactor < offsetWidth;
      this.centerV = (height * zoom) / scaleFactor < offsetHeight;
    },
    ...
    onDragCanvas(e) {
      ...
      dragInfo = {
        canvasContainer,
        startLeft: canvasContainer.scrollLeft,
        startTop: canvasContainer.scrollTop,
        startMouse: e,
      };
      window.addEventListener("mousemove", this.dragCanvas);
      window.addEventListener("mouseup", this.stopDragCanvas);
    },
    dragCanvas(e) {
      const { canvasContainer, startLeft, startTop, startMouse } = dragInfo;
      canvasContainer.scrollLeft = startLeft + startMouse.clientX - e.clientX;
      canvasContainer.scrollTop = startTop + startMouse.clientY - e.clientY;
    },
    stopDragCanvas() {
      window.removeEventListener("mousemove", this.dragCanvas);
      window.removeEventListener("mouseup", this.stopDragCanvas);
    },
